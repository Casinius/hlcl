...
* GPU kernel backend (`--gpu=y`): `--gpu_backend=omp|cuda` (default `omp` = OpenMP host, runs without a GPU).
  CUDA is direct PTX translation: needs an NVIDIA driver + CUDA toolkit on the build machine, plus `--cuda_arch=sm_XX` (GTX 16xx `sm_75`, RTX 30xx `sm_86`, RTX 40xx `sm_89`).
